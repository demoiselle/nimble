...
//	static String PATTERN = "%d{ABSOLUTE} %-5p %m%n"
	static String PATTERN = "%-5p %m%n"
	static Logger instance = null
	
	static Logger getInstance(clazz) {
		if (instance == null) {
			def simple = new PatternLayout(PATTERN)
			BasicConfigurator.configure(new ConsoleAppender(simple))
			//def appender = new FileAppender(PATTERN , "log/log.txt" , false)
