@rem ##########################################################################
@rem                                                                         ##
@rem  startDemoiselle for Windowz                                            ##
@rem                                                                         ##
@rem ##########################################################################
...
:check_DEMOISELLE_HOME
@rem Define DEMOISELLE_HOME if not set
if "%DEMOISELLE_HOME%" == "" set DEMOISELLE_HOME=%DIRNAME%..

@rem Remove trailing slash from DEMOISELLE_HOME if found
if "%DEMOISELLE_HOME:~-1%"=="\" SET DEMOISELLE_HOME=%DEMOISELLE_HOME:~0,-1%
...
for %%c in (%DEMOISELLE_HOME%\lib\*.jar) do set STARTER_CLASSPATH=%STARTER_CLASSPATH%;%%c;
@rem Setting a classpath using the -cp or -classpath option means not to use
@rem the global classpath.
@rem interpreter
...
set STARTER_MAIN_CLASS=br.gov.frameworkdemoiselle.tools.nimble.Main
...
@rem Execute Demoiselle Nimble
"%JAVA_EXE%" %JAVA_OPTS% -classpath "%STARTER_CLASSPATH%" %STARTER_MAIN_CLASS% %CMD_LINE_ARGS%
